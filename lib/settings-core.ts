...
const SOCIAL_LABELS: Record<'instagram' | 'facebook' | 'tiktok' | 'youtube', string> = {
  instagram: 'Instagram',
  facebook: 'Facebook',
  tiktok: 'TikTok',
  youtube: 'YouTube',
...
export function resolveSettings(stored: StoredSettings, fallback: SettingsFallback): ResolvedSettings {
  ...
  const storedSocial = (Object.keys(SOCIAL_LABELS) as (keyof typeof SOCIAL_LABELS)[])
    .filter((key) => stored[key])
    .map((key) => ({ label: SOCIAL_LABELS[key], href: stored[key] as string }));
