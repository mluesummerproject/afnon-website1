      ...
      <AnimatePresence>
        {items.map((item) => (
          <motion.div
            ...
            className="pointer-events-auto flex w-full max-w-md items-center gap-3 rounded-[14px] bg-ink px-4 py-3 text-[14px] leading-snug text-white shadow-sheet"
          >
            <span className="flex-1">{item.message}</span>
            {item.action ? (
              <button type="button" onClick={item.action.run} className="tap min-h-[44px] shrink-0 px-2 text-[14px] font-semibold text-white underline underline-offset-4">
                {item.action.label}
              </button>
            ) : null}
            <button
              ...
              className="-mr-2 flex h-11 w-11 shrink-0 items-center justify-center text-white/70"
            >
